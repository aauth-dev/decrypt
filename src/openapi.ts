// OpenAPI 3.1 for decrypt.agent.coop, served at /openapi.json.
export function openapi(origin: string) {
  const key = { $ref: '#/components/schemas/Key' }
  return {
    openapi: '3.1.0',
    info: {
      title: 'decrypt.agent.coop',
      version: '0.1.0',
      description:
        'Holds the private key that decrypts your secret.agent.coop messages and decrypts them for your agent. Keys are P-256 for JWE ECDH-ES with A256GCM. Identity is your Person Server (iss, sub), directed to this service; it never sees an address.',
    },
    servers: [{ url: origin }],
    paths: {
      '/key': {
        get: {
          operationId: 'getKey',
          summary: 'Your current public key, created if you have none. Register it at secret.agent.coop with addKey {kid, alg, jwk}.',
          responses: { '200': { description: 'The key', content: { 'application/json': { schema: key } } } },
        },
        post: {
          operationId: 'rotateKey',
          summary: 'Mint a new key. Older keys stay decryptable. Register the new one at secret.agent.coop.',
      ...
      '/decrypt': {
        post: {
          ...
          summary:
            'Decrypt one message. JSON body {protected, iv, tag, ciphertext} where ciphertext is base64url of the raw bytes from secret.agent.coop getMessage (its JSON form returns exactly these fields). Or application/octet-stream raw ciphertext with ?protected=&iv=&tag=. Returns {kid, size, plaintext, warnings}.',
