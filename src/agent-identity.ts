// decrypt's own agent identity (D1, D24). On getMessage, decrypt is an AAuth
...
//
// Copied from encrypt/src/agent-identity.ts with the sub changed; the later
// increment 7 (@aauth/intermediary) makes it one module.
//
// Two keys, on purpose:
...
//               issues to decrypt for the messaging service.
//
...
/** `aauth:read@<origin host>`: the one identity decrypt acts under. */
export function agentSub(origin: string): string {
  return `aauth:read@${new URL(origin).host}`
}
...
/**
 * decrypt's agent token, minted on demand and cached per isolate until five
