#!/usr/bin/env bash
# Live smoke for decrypt.aauth.dev: unauthenticated surface, the agent document, and the 401 shape.
#   bash scripts/smoke.sh [base_url]
set -uo pipefail
BASE="${1:-https://decrypt.aauth.dev}"
PASS=0; FAIL=0
...
check "openapi lists getKey, rotateKey, readMessage" "$(echo "$SPEC" | jq -e '.paths["/key"].get.operationId == "getKey" and .paths["/key"].post.operationId == "rotateKey" and .paths["/read"].post.operationId == "readMessage"' >/dev/null 2>&1 && echo true || echo false)"
